...
fn replace_pseudo(instructions: &mut [Instruction], symbols: &SymbolTable) -> u32 {
    ...
    let mut m = |op: &mut Operand| {
        match op {
            ...
            Operand::Psuedo(x) => match symbols.get(x).map_or(Linkage::None, |s| s.linkage) {
                Linkage::None => {
...
pub fn generate_assembly(program: &tacky::Program, symbols: &SymbolTable) -> Program {
    ...
    static_variables
        .sort_by(|lhs: &StaticVariable, rhs: &StaticVariable| lhs.name.cmp(&rhs.name));
    Program {
        definitions,
        static_variables,
    }
