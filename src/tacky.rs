...
use crate::{
    parser::{
        self, Block, BlockItem, Declaration, Expression, Statement, StorageClass,
        VariableDeclaration,
    },
    sema::{self, Linkage, SymbolTable},
};
...
fn lower_variable_declaration<'i>(decl: &VariableDeclaration, state: &mut State<'i>) {
    ...
    match symbol.linkage {
        Linkage::External | Linkage::Static | Linkage::Tentative => {
            let var = state
                .static_variables
                .entry(decl.name.clone())
                .or_insert_with(|| StaticVariable {
                    identifier: decl.name.clone(),
                    global: symbol.linkage == Linkage::External,
                    init: 0,
                });
            println!("decl {decl:#?}");
...
fn lower_function<'a>(
    ...
    Some(Function {
        ...
        global: function.storage.is_none_or(|s| s == StorageClass::Extern),
        params,
